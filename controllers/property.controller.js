...
const getPropertyDetail = async (req, res) => {
    ...
    let propertyExists = await Property.findOne({ _id: id }).populate('creator').populate('allReviews');
    propertyExists = await propertyExists.populate('allReviews.user');
...
const updateProperty = async (req, res) => {
    try {
        ...
        const phtotUrl = await cloudinary.uploader.upload(photo);

        await Property.findByIdAndUpdate({ _id: id }, { title, description, propertyType, location, price, photo: phtotUrl.url || photo});
